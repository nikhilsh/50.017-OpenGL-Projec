...
namespace
{
    ...
    void initSystem(int argc, char * argv[])
    {
        ...
        /*
        if (argc != 4){
            cout << "wrong number of variables declared!" << endl;
            exit(0);
        } else {
            ...
            stepsize = stof(argv[2]);
            
            if ( strncmp(argv[3], "sim", 3 ) == 0 ){
                system = new SimpleSystem(1);
                system_state=0;
            } else if ( strncmp(argv[3], "pen", 3 ) == 0 ){
                system = new PendulumSystem(length);
                system_state=1;
            } else if ( strncmp(argv[3], "clo", 3 ) == 0 ){
                system = new ClothSystem(length*length);
                system_state=2;
            }
...
//    glutSolidSphere(0.1f,10.0f,10.0f);
    ...
    void keyboardFunc( unsigned char key, int x, int y )
    {
        ...
        case 'S':
            system->swing = (!system->swing);
                cout << "toggle swing:\t" << system->swing << endl;
            break;
        case 'w':
        case 'W':
            cout << "toggle wireframe:\t" << system->wireframe << endl;
            system->wireframe = (!system->wireframe);
            if( length != 4){
                system->wireframe = true;
                cout << "smooth shading cloth mode only when length is 4" << endl;
            }
            
            break;
        case 'r':
        case 'R':
                system->setState(system->initState);
            cout << "reset" << endl;
            break;
        case 't':
        case 'T':
        ...
        case 'Q':
                ...
                cout << "toggle wind: \t" << system->wind << endl;
        default:
    ...
    void reshapeFunc(int w, int h)
    {
        ...
        camera.SetRotation(camera.GetRotation().rotateX(0.5));
